...
const handle_submit = () => {
  verifiedUser.value = user.value.filter(
    (item) => item.Email == selectedEmail.value && item.Password == selectedPassword.value
  )
  ...
  if (
    selectedEmail.value === allowedEmail.value &&
    selectedPassword.value === allowedPassword.value
  ) {
